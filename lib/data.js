...
export const ACTIVITIES = [
  // Paris
  { id: 1, city_id: 1, name: 'Eiffel Tower Visit', description: 'Ascend the iconic iron lattice tower for breathtaking panoramic views of Paris.', category: 'Sightseeing', estimated_cost: 26, duration: '2-3 hours', image_url: 'https://images.unsplash.com/photo-1511739001486-6bfe10ce65f4?w=600', rating: 4.8 },
...
export function loginUser(email, password) {
  ...
  if (supabase) {
    supabase.from('users').upsert([{
      ...
      if (error) console.log('Supabase user sync:', error.message);
    });
  }
...
export function signupUser(data) {
  const { firstName, lastName, email, password, city, country, phone } = data;
...
export function getUserTrips(userId) {
  return getTrips().filter(t => t.user_id === userId);
}
...
export function getCityActivities(cityId, filters = {}) {
  let results = ACTIVITIES.filter(a => a.city_id === cityId);
