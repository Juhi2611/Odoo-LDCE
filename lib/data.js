...
export function isAdminUser(user) {
  ...
  const adminEmails = [
    ...
    'admin@travel.com',
    'admin@odoo.com',
    'juhi@globetrotter.com',
    'nandish@globetrotter.com',
    'snehi@globetrotter.com',
    'yashvi@globetrotter.com'
  ];
...
export function loginUser(email, password) {
  ...
  const isSuperAdmin = cleanEmail.startsWith('admin@') || ['admin@globetrotter.com', 'admin@travel.com'].includes(cleanEmail);

  if (user) {
    // If admin email, guarantee admin role
    if (isSuperAdmin) {
      user.role = 'admin';
      user.is_admin = true;
    }
    ...
    user = {
      ...
      email: cleanEmail,
      name: isSuperAdmin ? 'System Administrator' : cleanEmail.split('@')[0].replace(/[._]/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
      avatar_url: isSuperAdmin ? 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=200' : null,
      city: isSuperAdmin ? 'San Francisco' : 'Mumbai',
      country: isSuperAdmin ? 'United States' : 'India',
      role: isSuperAdmin ? 'admin' : 'user',
      is_admin: isSuperAdmin,
      preferences: { currency: 'INR', language: 'en' },
      saved_city_ids: [1, 2, 3],
...
export function getSavedCityIds() {
  const user = getCurrentUser();
  if (!user) return getStorage('globetrotter_saved_cities', []);
  return user.saved_city_ids || [];
}

export function isCitySaved(cityId) {
  const saved = getSavedCityIds();
  return saved.includes(Number(cityId));
}

export function toggleSavedCity(cityId) {
  const user = getCurrentUser();
  const idNum = Number(cityId);
  
  if (user) {
    const current = user.saved_city_ids || [];
    const isSaved = current.includes(idNum);
    const updatedIds = isSaved ? current.filter(id => id !== idNum) : [...current, idNum];
    
    const updatedUser = { ...user, saved_city_ids: updatedIds };
    setStorage(STORAGE_KEYS.USER, updatedUser);
    
    if (user.email) {
      const cleanEmail = user.email.toLowerCase();
      const registeredUsers = getStorage(STORAGE_KEYS.REGISTERED_USERS, {});
      registeredUsers[cleanEmail] = updatedUser;
      setStorage(STORAGE_KEYS.REGISTERED_USERS, registeredUsers);
    }
    ...
    return { saved: !isSaved, saved_city_ids: updatedIds };
  } else {
    const current = getStorage('globetrotter_saved_cities', []);
    const isSaved = current.includes(idNum);
    const updated = isSaved ? current.filter(id => id !== idNum) : [...current, idNum];
    setStorage('globetrotter_saved_cities', updated);
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new Event('storage'));
    }
    return { saved: !isSaved, saved_city_ids: updated };
  }
}
